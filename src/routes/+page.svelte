<div class="flex justify-center">
    <div class="flex justify-center mt-[2rem]">
        <div class="w-[320px] h-[635px] border-[5px] border-[#c9c9c9] rounded-[45px] pr-4">
            <div class="w-[310px] h-[625px] border-[8px] border-black rounded-[40px] absolute justify-center items-center">
                <div class="w-[110px] h-[27px] absolute mt-[9px] ml-[6rem] rounded-full bg-black static animate-scaleOutIs active:animate-scaleIs">
                    <div class="w-[15px] h-[15px] bg-[#3d3d3d] mt-[6px] rounded-full ml-[85px] flex ">
                        <div class="w-[5px] h-[5px] bg-black mt-[5px] rounded-full ml-[5px]">
                        </div>
                    </div>
                </div>
                <img src="/images/wallpaper.jpeg" alt="wallpaper" class="w-[1000px] h-[609px] rounded-[30px] mx-auto my-auto">
            </div>
        </div>
    </div>
</div>
<style lang="postcss">
</style>